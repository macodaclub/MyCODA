...
sealed class MdTemplate(val fileName: String) {
    class SubmitChangesIssue(val changesJson: String, val addedEntitiesHtmlTables: String, val editedEntitiesHtmlTables: String) : MdTemplate("submit-changes-issue-template.md") {
        suspend fun get() = getMd(fileName)
            .replaceFirst("%changesJson%", changesJson)
            .replaceFirst("%addedEntitiesHtmlTables%", addedEntitiesHtmlTables)
            .replaceFirst("%editedEntitiesHtmlTables%", editedEntitiesHtmlTables)
    }
